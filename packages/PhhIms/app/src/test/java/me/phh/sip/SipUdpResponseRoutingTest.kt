...
class SipUdpResponseRoutingTest {
    ...
    @Test
    fun `packet source policy keeps protected response on inbound flow`() {
        val bytes = response("SIP/2.0/UDP [2001:db8::30]:5067;branch=z9")
        val route = SipUdpResponseRouting.route(
            responseBytes = bytes,
            sourceAddress = packetSource,
            sourcePort = 32922,
            targetPolicy = SipUdpResponseTargetPolicy.PACKET_SOURCE,
        )
